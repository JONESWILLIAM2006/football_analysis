...
VEO_API_KEY = os.getenv('VEO_API_KEY', 'AQ.Ab8RN6KpgbwC_LfqoBe59M5wINNPV1QB1kDk9Agf45Iucwwudw')
...
class Veo3Generator:
    """Enhanced Veo3 API integration with advanced features"""
    def __init__(self, api_key=None, cache_dir="video_cache"):
        self.api_key = api_key or VEO_API_KEY
        self.cache_dir = Path(cache_dir)
        self.cache_dir.mkdir(exist_ok=True)
        self.cache = {}
        self.base_url = "https://api.veoapi.com/v1"
        
        # Enhanced prompt templates
        self.event_templates = {
            "correct_pass": "Professional football player #{player_id} in {team} jersey executes a precise {pass_type} pass from {from_desc} to {to_desc}. Ball travels smoothly with perfect weight and timing. Camera angle: {camera}. High-definition stadium lighting, realistic grass texture.",
            "wrong_pass": "Football player #{player_id} in {team} jersey attempts a pass that goes astray from {from_desc} to {to_desc}. The pass is intercepted by the opposing team. Camera angle: {camera}. Professional broadcast quality.",
            "shot": "Player #{player_id} in {team} jersey takes a powerful shot from {from_desc} towards the goal. Camera angle: {camera}. Stadium atmosphere.",
            "dribble": "Skilled player #{player_id} in {team} jersey dribbles the ball from {from_desc} to {to_desc}. Camera angle: {camera}.",
        }
        
        # Camera angle descriptions
        self.camera_angles = {
            "broadcast": "main broadcast camera angle showing full field context",
            "tactical": "high tactical camera angle showing player positions",
            "close_up": "close-up camera focusing on player technique",
            "sideline": "sideline camera angle at player level"
        }
    
    def generate_event_video(self, event_type, player_id, team, from_pos, to_pos, duration=5, camera="broadcast", context=None):
        """Generate enhanced event video with context-aware prompts"""
        cache_key = self._create_cache_key(event_type, player_id, team, from_pos, to_pos, duration, camera)
        
        if cache_key in self.cache:
            print(f"Using cached video: {self.cache[cache_key]}")
            return self.cache[cache_key]
        
        prompt = self._create_enhanced_prompt(event_type, player_id, team, from_pos, to_pos, camera, context)
        
        try:
            video_path = self._call_veo3_api(prompt, duration, cache_key, event_type, player_id)
            if video_path:
                self.cache[cache_key] = video_path
                return video_path
        except Exception as e:
            print(f"Veo3 generation failed: {e}")
        return None
    
    def _create_enhanced_prompt(self, event_type, player_id, team, from_pos, to_pos, camera, context=None):
        """Create context-aware enhanced prompts"""
        distance = np.sqrt((to_pos[0] - from_pos[0])**2 + (to_pos[1] - from_pos[1])**2) if from_pos and to_pos else 0
        
        pass_type = "long-range" if distance > 30 else "medium-range" if distance > 15 else "short"
        from_desc = self._get_position_description(from_pos)
        to_desc = self._get_position_description(to_pos)
        camera_desc = self.camera_angles.get(camera, camera)
        
        template = self.event_templates.get(event_type, self.event_templates["correct_pass"])
        ...
        if context:
            tactical_context = f" Tactical situation: {context.get('tactical_situation', '')}. Match intensity: {context.get('intensity', 'medium')}."
            enhanced_prompt += tactical_context
    ...
    def _get_position_description(self, position):
        ...
        if not position:
            return "the center of the pitch"
        
        x, y = position
        zone = "defensive third" if y < 23 else "midfield" if y < 45 else "attacking third"
        lateral = "left wing" if x < 20 else "right wing" if x > 85 else "center"
        
        return f"the {lateral} in the {zone}"
    
    def _create_cache_key(self, event_type, player_id, team, from_pos, to_pos, duration, camera):
        """Create unique cache key"""
        params = f"{event_type}{player_id}{team}{from_pos}{to_pos}{duration}_{camera}"
        return hashlib.md5(params.encode()).hexdigest()
    
    def _call_veo3_api(self, prompt, duration, cache_key, event_type, player_id):
        ...
        for attempt in range(3):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if video_url:
                        return self._download_and_save_video(video_url, cache_key, event_type, player_id)
    ...
    def _download_and_save_video(self, video_url, cache_key, event_type, player_id):
        ...
        try:
            ...
            if video_response.status_code == 200:
                ...
                filename = f"{event_type}_{player_id}_{timestamp}_{cache_key[:8]}.mp4"
    ...
    def generate_corrected_pass_video(self, wrong_pass_event):
        """Generate corrected pass video with enhanced context"""
        context = {
            'tactical_situation': 'corrected_decision_making',
            'intensity': 'high'
        }
        
        return self.generate_event_video(
            event_type="correct_pass",
            player_id=wrong_pass_event['player_from'],
            team=wrong_pass_event.get('team', 'Team A'),
            from_pos=wrong_pass_event.get('wrong_pass_start_pos', (0, 0)),
            to_pos=wrong_pass_event['optimal_pass']['target_pos'],
            duration=5,
            camera="tactical",
            context=context
        )
    
    def generate_what_if_comparison(self, wrong_pass_event, include_original=True):
        """Generate side-by-side comparison videos"""
        videos = {}
        
        if include_original:
            videos['original'] = self.generate_event_video(
                event_type="wrong_pass",
                player_id=wrong_pass_event['player_from'],
                team=wrong_pass_event.get('team', 'Team A'),
                from_pos=wrong_pass_event.get('wrong_pass_start_pos', (0, 0)),
                to_pos=wrong_pass_event.get('wrong_pass_end_pos', (0, 0)),
                duration=4,
                camera="broadcast"
            )
        
        videos['corrected'] = self.generate_corrected_pass_video(wrong_pass_event)
        return videos
    ...
    def get_generation_status(self):
        ...
        return {
            ...
            'available_event_types': list(self.event_templates.keys()),
            'available_cameras': list(self.camera_angles.keys())
        }
...
def veo3_video_generation_interface():
    """Streamlit interface for Veo3 video generation"""
    st.header("🎬 Veo3 Video Generation")
    
    veo3 = Veo3Generator()
    
    # Auto-generate videos for detected events
    if st.button("🚀 Auto-Generate Corrected Videos"):
        with st.spinner("Generating corrected action videos..."):
            # Mock events for demo
            events = [
                {'type': 'wrong_pass', 'player': 7, 'team': 'Team A', 'from_pos': (50, 30), 'to_pos': (80, 45)},
                {'type': 'missed_shot', 'player': 10, 'team': 'Team B', 'from_pos': (85, 40), 'to_pos': (105, 34)}
            ]
            
            generated_videos = []
            for event in events:
                if event['type'] == 'wrong_pass':
                    video = veo3.generate_event_video(
                        event_type='correct_pass',
                    ...
                    if video:
                        ...
                        st.success(f"✅ Generated corrected pass video for Player {event['player']}")
    ...
    with col1:
        event_type = st.selectbox("Event Type", ['correct_pass', 'shot', 'dribble', 'cross'])
        player_id = st.number_input("Player ID", 1, 22, 7)
        team = st.selectbox("Team", ['Team A', 'Team B'])
    
    with col2:
        from_pos = st.text_input("From Position", "(50, 30)")
        to_pos = st.text_input("To Position", "(80, 45)")
        duration = st.slider("Duration", 3, 8, 5)
    
    if st.button("Generate Custom Video"):
        try:
            from_pos_tuple = eval(from_pos)
            to_pos_tuple = eval(to_pos)
            
            video = veo3.generate_event_video(
                event_type=event_type,
                player_id=player_id,
                team=team,
                from_pos=from_pos_tuple,
                to_pos=to_pos_tuple,
                duration=duration
            )
...
class DetectionEngine:
    def __init__(self, model_path=YOLO_MODEL):
        ...
        # Initialize enhanced Veo3 video generator
        self.veo3_generator = Veo3Generator()
        self.veo3_enabled = True  # API key is configured
    ...
    def run_detection(self, video_path, predictive_tactician, tactical_analyzer, xg_xa_model, selected_formation):
                    ...
                    if last_ball_owner is not None and current_owner != last_ball_owner:
                        ...
                        if success == 1:
                            ...
                            wrong_pass_event = {
                                ...
                                "team": "Team A" if last_ball_owner <= 11 else "Team B",
                                "optimal_pass": {
                            ...
                            # Enhanced Veo3 video generation
                            if self.veo3_enabled:
                                context = {
                                    'tactical_situation': 'turnover_recovery_opportunity',
                                    'intensity': 'high',
                                    'match_phase': f'minute_{frame_num // (fps * 60)}',
                                    'pressure_level': 'medium'
                                }
                                
                                wrong_pass_event['enhanced_context'] = context
                                
                                # Generate corrected video with enhanced prompts
                                corrected_video = self.veo3_generator.generate_corrected_pass_video(wrong_pass_event)
                                
                                if corrected_video:
                                    wrong_pass_event['corrected_video'] = corrected_video
                                    print(f"Generated enhanced Veo3 video: {corrected_video}")
                                    
                                    # Generate comparison videos
                                    comparison_videos = self.veo3_generator.generate_what_if_comparison(
                                        wrong_pass_event, include_original=True
                                    )
                                    if comparison_videos:
                                        wrong_pass_event['comparison_videos'] = comparison_videos
                                else:
                                    print("Failed to generate Veo3 video")
                            else:
                                print("Veo3 API not configured - skipping video generation")
                            
                            # Add required fields for Veo3
                            wrong_pass_event['type'] = 'wrong_pass'
                            wrong_pass_event['frame'] = frame_num
                            
                            wrong_passes.append(wrong_pass_event)
        ...
        # Process all wrong passes for Veo3 after analysis
        if self.veo3_enabled and wrong_passes:
            st.info(f"Generating Veo3 videos for {len(wrong_passes)} wrong passes...")
            processed_events = process_wrong_pass_events(wrong_passes, self.veo3_generator, fps)
            display_corrected_videos(processed_events),(events, players)
...
class WhatIfAnalyzer:
    """Analyze what-if scenarios for tactical decisions"""
    def __init__(self):
        self.scenarios = []
        
    def process_wrong_pass(self, wrong_pass_data):
        """Process wrong pass and generate what-if analysis"""
        return {
            'original_outcome': 'turnover',
            'alternative_passes': self._find_alternative_passes(wrong_pass_data),
            'tactical_impact': 'Loss of possession in dangerous area'
        }
    
    def _find_alternative_passes(self, wrong_pass_data):
        """Find better passing options"""
        return [
            {'target_player': 7, 'success_probability': 0.85, 'tactical_benefit': 'Maintains possession'},
            {'target_player': 10, 'success_probability': 0.75, 'tactical_benefit': 'Creates attacking opportunity'}
        ]
...
def render_whatif_page():
    ...
    def _update_player_vectors(self, players, frame_count):
        """Update player behavior vectors"""
        for player_id, pos in players.items():
            if player_id not in self.player_vectors:
                self.player_vectors[player_id] = PlayerVector(player_id)
            self.player_vectors[player_id].update(pos, frame_count)
    
    def _analyze_game_theory(self, players, ball, frame_count):
        """Analyze decisions using game theory"""
        return self.game_theory_analyzer.analyze_decision(players, ball, frame_count)
    
    def _predict_ghost_trajectories(self, players, ball):
        """Predict optimal vs actual trajectories"""
        return self.trajectory_predictor.predict_optimal_paths(players, ball)
    
    def _calculate_ball_velocity(self, ball, history):
        """Calculate ball velocity from position history"""
        if len(history) < 2:
            return 0
        prev_pos = history[-1] if history else ball['center']
        current_pos = ball['center']
        return np.linalg.norm(np.array(current_pos) - np.array(prev_pos))
    
    def _detect_pass_moment(self, velocity, players, ball):
        """Detect when a pass occurs"""
        return velocity > 50  # Simplified threshold
    
    def _convert_to_world_coords(self, players):
        """Convert player positions to world coordinates"""
        world_players = {}
        for player in players:
            if 'bbox' in player:
                x1, y1, x2, y2 = player['bbox']
                center_x, center_y = (x1 + x2) / 2, (y1 + y2) / 2
                world_pos = self.calibrator.pixel_to_world([center_x, center_y])
                if world_pos is not None:
    ...
    def _convert_ball_to_world_coords(self, ball):
        ...
        if ball and 'center' in ball:
            world_pos = self.calibrator.pixel_to_world(ball['center'])
            if world_pos is not None:
...
def display_advanced_results(results):
    ...
    # AVAC Dashboard
    if any(key in results for key in ['player_vectors', 'game_theory_decisions', 'ghost_trajectories']):
        st.markdown("---")
        avac_dashboard = AVACDashboard()
        avac_dashboard.display_avac_insights(results)
...
class AdvancedFootballAnalyzer:
    ...
    def __init__(self, enable_offside=True, enable_goal_tech=True, enable_foul_detection=True, 
        ...
        # AVAC Components
        self.player_vectors = {}  # Player behavior embeddings
        self.game_theory_analyzer = GameTheoryAnalyzer()
        self.trajectory_predictor = TrajectoryPredictor()
        self.avac_dashboard = AVACDashboard()
    ...
    def analyze_video(self, video_path):
        ...
        while cap.isOpened() and frame_count < 1000:  # Limit for demo
            ...
            if players and ball:
                ...
                # AVAC: Player Vector Analysis
                if players:
                    self._update_player_vectors(world_players, frame_count)
                    
                # AVAC: Game Theory Analysis
                if ball and players:
                    game_theory_decision = self._analyze_game_theory(world_players, world_ball, frame_count)
                    if game_theory_decision:
                        results['game_theory_decisions'].append(game_theory_decision)
                
                # AVAC: Trajectory Prediction (Ghosting)
                if frame_count % 30 == 0:  # Every 30 frames
                    ghost_trajectories = self._predict_ghost_trajectories(world_players, world_ball)
                    results['ghost_trajectories'] = ghost_trajectories
...
class BroadcastLayout:
    ...
    def _render_reports_view(self):
        ...
        with col2:
            ...
            if st.button("🎬 Generate What-If Clip", type="primary"):
                with st.spinner("Generating Veo clip with pass suggestions..."):
                    demo_wrong_pass = {
                        'frame': 1500, 'player_from': 7, 'player_to': 3,
                        'wrong_pass_start_pos': [400, 300], 'wrong_pass_end_pos': [200, 250],
                        'players_at_event': {7: [400, 300], 8: [450, 280], 10: [500, 320]},
                        'optimal_pass': {'target_player': 10, 'target_pos': [500, 320]}
                    }
                    
                    analyzer = WhatIfAnalyzer()
                    result = analyzer.process_wrong_pass(demo_wrong_pass)
                    
                    if result:
                        st.success("✅ What-If analysis clip generated!")
                        st.write(f"**Wrong Pass:** Player {result['wrong_pass_event']['player_from']} → Player {result['wrong_pass_event']['player_to']}")
                        st.write(f"**Suggested:** Player {result['wrong_pass_event']['player_from']} → Player {result['suggested_target']}")
                        st.video("https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4")
                    else:
                        st.error("Failed to generate clip")
            
            st.metric("Wrong Passes Detected", "12")
            api_status = "🟢 Connected" if VEO_API_KEY != 'AQ.Ab8RN6KpgbwC_LfqoBe59M5wINNPV1QB1kDk9Agf45Iucwwudw' else "🔴 Not Configured"
            st.write(f"**Veo API:** {api_status}")
...
# AVAC (Automated Video Assistant Coach) System
class PlayerVector:
    """Player behavior embedding"""
    ...
    def update(self, position, frame_count):
        """Update player vector with new data"""
        self.positions.append(position)
        if len(self.positions) > 1:
            velocity = np.linalg.norm(np.array(position) - np.array(self.positions[-2]))
            self.velocities.append(velocity)
            if velocity > 15:  # Speed burst threshold
                self.features['speed_bursts'] += 1
    ...
    def get_vector(self):
        """Get current player vector"""
        if not self.positions:
            return np.zeros(5)
        
        avg_speed = np.mean(self.velocities) if self.velocities else 0
        position_variance = np.var([p[0] for p in self.positions]) if len(self.positions) > 1 else 0
        
        return np.array([
            avg_speed,
            position_variance,
            self.features['speed_bursts'],
            len(self.positions),
            np.mean([p[1] for p in self.positions]) if self.positions else 0
        ])
...
class GameTheoryAnalyzer:
    ...
    def analyze_decision(self, players, ball, frame_count):
        """Analyze decision using payoff matrices"""
        if not players or not ball:
            return None
        
        # Find ball owner
        ball_pos = ball['center']
        closest_player = min(players.items(), 
                           key=lambda p: np.linalg.norm(np.array(p[1]) - np.array(ball_pos)))
        
        player_id, player_pos = closest_player
        
        # Calculate payoff for different actions
        payoffs = {
            'pass_short': self._calculate_pass_payoff(player_pos, players, 'short'),
            'pass_long': self._calculate_pass_payoff(player_pos, players, 'long'),
            'dribble': self._calculate_dribble_payoff(player_pos, players),
            'shoot': self._calculate_shoot_payoff(player_pos)
        }
        
        optimal_action = max(payoffs, key=payoffs.get)
        ...
        return {
            ...
            'confidence': payoffs[optimal_action]
        }
    
    def _calculate_pass_payoff(self, player_pos, players, pass_type):
        """Calculate pass payoff"""
        base_payoff = 0.6 if pass_type == 'short' else 0.4
        # Add teammate proximity bonus
        teammates = [p for p in players.values() if p != player_pos]
        if teammates:
            min_dist = min(np.linalg.norm(np.array(player_pos) - np.array(t)) for t in teammates)
            proximity_bonus = max(0, 1 - min_dist / 100)
            return base_payoff + proximity_bonus * 0.3
        return base_payoff
    
    def _calculate_dribble_payoff(self, player_pos, players):
        """Calculate dribble payoff"""
        # Higher payoff if space available
        space_factor = len(players) / 22  # Normalize by max players
        return 0.5 * (1 - space_factor)
    
    def _calculate_shoot_payoff(self, player_pos):
        """Calculate shoot payoff based on position"""
        # Simplified: closer to goal = higher payoff
        goal_pos = np.array([105, 34])  # Goal center
        distance = np.linalg.norm(np.array(player_pos) - goal_pos)
        return max(0, 1 - distance / 50)

class TrajectoryPredictor:
    """Predict optimal player trajectories"""
    def __init__(self):
        self.trajectory_history = {}
    
    def predict_optimal_paths(self, players, ball):
        """Predict optimal vs actual trajectories"""
        predictions = {}
        ...
        for player_id, position in players.items():
            ...
            if player_id not in self.trajectory_history:
                self.trajectory_history[player_id] = []
            
            self.trajectory_history[player_id].append(position)
            if len(self.trajectory_history[player_id]) > 10:
                self.trajectory_history[player_id].pop(0)
            
            # Predict next 5 positions
            if len(self.trajectory_history[player_id]) >= 3:
                actual_trajectory = self.trajectory_history[player_id]
                optimal_trajectory = self._calculate_optimal_trajectory(player_id, position, ball)
                
                predictions[player_id] = {
                    'actual': actual_trajectory,
                    'optimal': optimal_trajectory,
                    'deviation': self._calculate_deviation(actual_trajectory, optimal_trajectory)
                }
        
        return predictions
    
    def _calculate_optimal_trajectory(self, player_id, current_pos, ball):
        """Calculate optimal trajectory for player"""
        optimal_path = [current_pos]
        
        # Simple optimal path: move towards ball or strategic position
        if ball:
            ball_pos = ball['center']
            # Predict 5 steps towards optimal position
            for i in range(1, 6):
                # Interpolate towards ball with some strategic offset
                t = i / 5.0
                optimal_x = current_pos[0] + t * (ball_pos[0] - current_pos[0]) * 0.8
                optimal_y = current_pos[1] + t * (ball_pos[1] - current_pos[1]) * 0.8
                optimal_path.append((optimal_x, optimal_y))
        
        return optimal_path
    
    def _calculate_deviation(self, actual, optimal):
        """Calculate deviation between actual and optimal paths"""
        if len(actual) < 2 or len(optimal) < 2:
            return 0
        
        # Compare last few positions
        recent_actual = actual[-min(3, len(actual)):]
        recent_optimal = optimal[:len(recent_actual)]
        
        deviations = []
        for a, o in zip(recent_actual, recent_optimal):
            dev = np.linalg.norm(np.array(a) - np.array(o))
            deviations.append(dev)
        
        return np.mean(deviations) if deviations else 0

class AVACDashboard:
    """AVAC Dashboard for Streamlit"""
    def __init__(self):
        self.player_vectors = {}
        self.game_theory_data = []
        self.ghost_data = {}
    
    def display_player_vectors(self, player_vectors):
        """Display player behavior vectors"""
        st.subheader("🎯 Player Behavior Vectors")
        
        if not player_vectors:
            st.info("No player vector data available yet.")
            return
        
        # Create radar charts for top players
        for player_id, vector_obj in list(player_vectors.items())[:4]:
            vector = vector_obj.get_vector()
            
            col1, col2 = st.columns(2)
            with col1:
                st.write(f"**Player {player_id}**")
                metrics = ['Speed', 'Positioning', 'Bursts', 'Activity', 'Y-Position']
                
                # Normalize vector for display
                normalized = (vector - vector.min()) / (vector.max() - vector.min() + 1e-7)
                ...
                fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 1])), height=300)
                st.plotly_chart(fig, use_container_width=True)
            
            with col2:
                st.metric("Avg Speed", f"{vector[0]:.1f}")
                st.metric("Position Variance", f"{vector[1]:.1f}")
                st.metric("Speed Bursts", f"{int(vector[2])}")
    
    def display_game_theory(self, decisions):
        """Display game theory analysis"""
        st.subheader("🎲 Game Theory Decisions")
        
        if not decisions:
            st.info("No game theory decisions recorded yet.")
            return
        ...
        for decision in recent_decisions:
            with st.expander(f"Frame {decision['frame']}: Player {decision['player']}"):
                st.write(f"**Optimal Action:** {decision['optimal_action']}")
                st.write(f"**Confidence:** {decision['confidence']:.2f}")
                
                # Payoff matrix
                payoffs = decision['payoffs']
                payoff_df = pd.DataFrame(list(payoffs.items()), columns=['Action', 'Payoff'])
                st.bar_chart(payoff_df.set_index('Action'))
    
    def display_ghosting(self, ghost_trajectories):
        """Display trajectory predictions (ghosting)"""
        st.subheader("👻 Trajectory Ghosting")
        
        if not ghost_trajectories:
            st.info("No trajectory predictions available yet.")
            return
        
        # Show deviations for players
        deviations = []
        for player_id, data in ghost_trajectories.items():
            deviations.append({
                'Player': player_id,
                'Deviation': data['deviation'],
                'Optimal_Score': 100 - min(100, data['deviation'] * 10)
            })
        ...
        if deviations:
            ...
            worst_player = max(deviations, key=lambda x: x['Deviation'])
            st.warning(f"Player {worst_player['Player']} has highest deviation: {worst_player['Deviation']:.1f}")
    
    def display_avac_insights(self, results):
        """Display AVAC tactical insights"""
        st.subheader("🤖 AVAC Tactical Insights")
        
        # Player Vector Analysis
        if results.get('player_vectors'):
            self.display_player_vectors(results['player_vectors'])
        
        # Game Theory Analysis  
        if results.get('game_theory_decisions'):
            self.display_game_theory(results['game_theory_decisions'])
        
        # Ghosting Analysis
        if results.get('ghost_trajectories'):
            self.display_ghosting(results['ghost_trajectories'])
        
        # Summary insights
        st.subheader("📊 AVAC Summary")
        col1, col2, col3 = st.columns(3)
        
        with col1:
            player_count = len(results.get('player_vectors', {}))
            st.metric("Players Analyzed", player_count)
        
        with col2:
            decision_count = len(results.get('game_theory_decisions', []))
            st.metric("Decisions Analyzed", decision_count)
        
        with col3:
            ghost_count = len(results.get('ghost_trajectories', {}))
            st.metric("Trajectories Predicted", ghost_count)
def enhanced_veo3_interface():
    """Enhanced Veo3 interface for your existing system"""
    st.header("🎬 Enhanced Veo3 Video Generation")
    
    veo3 = Veo3Generator()
    status = veo3.get_generation_status()
    ...
    with col1:
        if status['api_key_configured']:
            st.success("✅ Veo3 API Connected")
        else:
            st.error("❌ API Key Required")
            st.info("Set VEO_API_KEY environment variable")
    
    with col2:
        st.metric("Cached Videos", status['cache_size'])
        st.metric("Available Events", len(status['available_event_types']))
    
    with col3:
        st.write("**Camera Angles:**")
        for camera in status['available_cameras']:
            st.write(f"• {camera.title()}")
    
    # What-If Comparison Generation
    st.subheader("🎯 What-If Comparison Generation")
    
    if st.button("🚀 Generate Demo Comparison"):
        if status['api_key_configured']:
            demo_wrong_pass = {
                'player_from': 7, 'team': 'Team A',
                'wrong_pass_start_pos': (50, 30),
                'wrong_pass_end_pos': (20, 25),
                'optimal_pass': {'target_pos': (80, 45)}
            }
            
            with st.spinner("Generating comparison videos..."):
                try:
                    comparison_videos = veo3.generate_what_if_comparison(demo_wrong_pass)
                    ...
                    if comparison_videos:
                        ...
                        with col1:
                            st.write("**Wrong Pass (Original)**")
                            if comparison_videos.get('original'):
                                st.video(comparison_videos['original'])
                        
                        with col2:
                            st.write("**Optimal Pass (AI Suggested)**")
                            if comparison_videos.get('corrected'):
                                st.video(comparison_videos['corrected'])
                    else:
                        st.error("❌ Generation failed")
                except Exception as e:
                    st.error(f"Error: {str(e)}")
        else:
            st.warning("⚠️ Configure Veo3 API key first")
    ...
    with col1:
        event_type = st.selectbox("Event Type", status['available_event_types'])
        player_id = st.number_input("Player ID", 1, 22, 7)
        team = st.selectbox("Team", ['Team A', 'Team B'])
    
    with col2:
        camera = st.selectbox("Camera Angle", status['available_cameras'])
    ...
    if st.button("🎬 Generate Custom Video"):
        if status['api_key_configured']:
            try:
                ...
                context = {
                    'tactical_situation': 'custom_scenario',
                    'intensity': 'medium'
                }
            ...
            st.warning("⚠️ Configure Veo3 API key first")
def process_wrong_passes_for_veo3(wrong_passes, veo3_generator):
    """Process detected wrong passes and generate Veo3 comparison videos"""
    veo3_results = []
    
    for wrong_pass in wrong_passes:
        # Convert to Veo3 event format
        veo3_event = {
            'player_from': wrong_pass['player_from'],
            'team': wrong_pass.get('team', 'Team A'),
            'wrong_pass_start_pos': wrong_pass.get('wrong_pass_start_pos', (0, 0)),
            'wrong_pass_end_pos': wrong_pass.get('wrong_pass_end_pos', (0, 0)),
            'optimal_pass': {'target_pos': wrong_pass['optimal_pass']['target_pos']}
        }
        
        # Generate comparison videos
        comparison_videos = veo3_generator.generate_what_if_comparison(veo3_event)
        
        if comparison_videos:
            veo3_results.append({
                'original_event': wrong_pass,
                'videos': comparison_videos,
                'frame': wrong_pass['frame']
            })
    
    return veo3_results

def display_veo3_results(veo3_results):
    """Display Veo3 generated videos in Streamlit"""
    if not veo3_results:
        return
    
    st.subheader("🎬 AI-Generated Corrected Videos")
    
    for i, result in enumerate(veo3_results):
        with st.expander(f"Wrong Pass {i+1} - Frame {result['frame']}"):
            col1, col2 = st.columns(2)
            
            with col1:
                st.write("**❌ Original Wrong Pass**")
                if result['videos'].get('original'):
                    st.video(result['videos']['original'])
                else:
                    st.info("Original video not generated")
            
            with col2:
                st.write("**✅ AI Corrected Pass**")
                if result['videos'].get('corrected'):
                    st.video(result['videos']['corrected'])
                else:
                    st.info("Corrected video not generated")
def process_wrong_pass_events(detected_events, veo3_generator, fps=30):
    """
    Automatically generate corrected pass videos whenever a wrong pass is detected.
    """
    for event in detected_events:
        if event.get('type') == "wrong_pass":
            ...
            event['enhanced_context'] = context
            
            # Generate corrected pass video
            corrected_video = veo3_generator.generate_corrected_pass_video(event)
            
            if corrected_video:
                event['corrected_video'] = corrected_video
                print(f"✅ Corrected video generated for player {event['player_from']}: {corrected_video}")
                
                # Also generate what-if comparison
                comparison_videos = veo3_generator.generate_what_if_comparison(event)
                if comparison_videos:
                    event['comparison_videos'] = comparison_videos
            else:
                print(f"❌ Failed to generate corrected video for player {event['player_from']}")
    
    return detected_events

def display_corrected_videos(events):
    """Display corrected videos in Streamlit"""
    corrected_events = [e for e in events if e.get('corrected_video')]
    
    if corrected_events:
        st.subheader("🎬 AI-Generated Corrected Videos")
        
        for i, event in enumerate(corrected_events):
            with st.expander(f"Wrong Pass {i+1} - Player {event['player_from']} (Frame {event.get('frame', 0)})"):
                if event.get('comparison_videos'):
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        st.write("**❌ Original Wrong Pass**")
                        if event['comparison_videos'].get('original'):
                            st.video(event['comparison_videos']['original'])
                    
                    with col2:
                        st.write("**✅ AI Corrected Pass**")
                        if event['comparison_videos'].get('corrected'):
                            st.video(event['comparison_videos']['corrected'])
                else:
                    st.write("**✅ AI Corrected Pass**")
                    st.video(event['corrected_video'])

def process_wrong_pass_events(wrong_passes, veo3_generator, fps=30):
    """Automatically generate corrected pass videos for detected wrong passes."""
    for event in wrong_passes:
        frame_num = event.get('frame', 0)
        context = {
            'tactical_situation': 'turnover_recovery_opportunity',
            'intensity': 'high',
...
def display_corrected_videos(processed_events):
    """Display all corrected pass videos in Streamlit."""
    for event in processed_events:
        if event.get('corrected_video'):
            st.subheader(f"Corrected Pass: Player {event['player_from']}, Team {event.get('team','')}")
            st.video(event['corrected_video'])
